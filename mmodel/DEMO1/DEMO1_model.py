...
class DEMO1Model(TrainableModule):
    ...
    def get_loss(self, imgs, labels=None, t=True):
        ...
        """ classify loss """
        if t:
            domain_preds_dc = 1- domain_preds_dc
            c =  domain_preds_dc / domain_preds_dc.sum()
            c = c * 36
            loss_cls = ent(self.C(feats_c), c)
            loss_cls_dc = ent(self.C(feats_dc), c)
            loss_cls_dd = ent(self.C(feats_dd), c)
    ...
    def _train_process(self, datas):
        ...
        L_dd_dc_diff = L("mut", "dd_dc_diff", 0.00001)
        # doms
        L_dis_d = L("dom", "dis_d")
    ...
    def _eval_process(self, datas):
        ...
        predcition[domain_preds_d > t] = self.cls_info["cls_num"]
        return predcition
